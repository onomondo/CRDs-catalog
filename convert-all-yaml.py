...
def write_schema_file(schema, dir, filename):
    schemaJSON = ""
...
if __name__ == "__main__":
  ...
  for crdPath in sys.argv[1:]:
    files = os.listdir(crdPath)
    
    for file in files:
        openapi_dir = file.split("_")[0] 
        if os.path.exists(openapi_dir):
            print("Directory exists, continuing")   
        else:
            os.mkdir(openapi_dir)
        f = open(file)
        with f:
          defs = []
              ...
              if "spec" in y and "versions" in y["spec"] and y["spec"]["versions"]:
                  for version in y["spec"]["versions"]:
                      if "schema" in version and "openAPIV3Schema" in version["schema"]:
                          ...
                          write_schema_file(schema, openapi_dir, filename)
